...
| **Bounded place pool** | Every place is carried whole through Routing and the Critic, so a bulk search returning ~50 was paid for three times over. The pool is capped at 15 (`TRIP_MAX_PLACES`) and the cap is spent a rank at a time across the interests, so a small pool stays balanced. |
| **No deterministic-tool loops** | `read_agent_results` re-reads state that cannot change mid-node, so a model reading it as "not done yet" looped on it until the step budget ran out — and the run never reached the Calendar stage. It now answers once per run, the sub-agent has its own step limit, and the itinerary can be composed from state, so the plan ships either way. |
| **Every agent bounded** | A sub-agent invoked without a `recursion_limit` inherits the graph's 80, so one looping agent costs ~40 model calls before anything stops it. `factory.STEP_LIMITS` sizes each agent by the work its prompt actually asks for, and `run_agent` is the single path all eleven go through — so a prose-only answer raises a named `AgentOutputError` instead of `KeyError: 'structured_response'`. |
